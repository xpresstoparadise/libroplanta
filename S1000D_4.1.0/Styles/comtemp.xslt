...
<xsl:template match="spareDescr">
  ...
  <xsl:template match="@limitCond">
    <Span>
      ...
      <Run Foreground="{$autoTextColor}">
        <xsl:text> [</xsl:text>
        <xsl:value-of select="xef:GetTextGroup($doc_language,$doc_country,'tlp_comtemp_limit_cond','comtemp.xslt')"></xsl:value-of>
        <xsl:text>: </xsl:text>
      </Run>
      <Run Foreground="{$autoTextColor}" FontSize="14" Event="AttributeEditor" MouseCursor="Hand" FontWeight="Bold">
        <xsl:value-of select="."></xsl:value-of>
  ...
  <xsl:template match="trigger">
    <Section>
      ...
      <Paragraph>
        <xsl:call-template name="before"></xsl:call-template>
        <Span Id="ChildrenAttributesContainer" Test="boolean(local-name()!='releaseEvent')">
          <xsl:apply-templates select="@*[local-name()!='releaseEvent']"></xsl:apply-templates>
        </Span>
        <Run FontSize="14" Foreground="{$autoTextColor}">
  ...
  <xsl:template match="threshold">
    <Span>
      <xsl:call-template name="idtemplate" />
      <xsl:call-template name="before"></xsl:call-template>
      <Span Id="ChildrenAttributesContainer" Test="boolean(local-name()='thresholdType')">
        <xsl:apply-templates select="@*[local-name()='thresholdType']"></xsl:apply-templates>
      </Span>
      <xsl:call-template name="first"></xsl:call-template>
