...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xef="http://www.libroplanta.com/xmlEditorFunctions" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="fn xs xef msxsl">
  ...
  <xsl:key name="brLevelledParas" match="brLevelledPara[@id]" use="@id" />	
    ...
    <Root>
      <xsl:for-each select="$node">
        <xsl:choose>
          ...
          <xsl:when test="$templateName='brLevelledParaNumber'">
            <xsl:call-template name="brLevelledParaNumber"></xsl:call-template>
          </xsl:when>				
          ...
          <xsl:when test="$templateName='derivativeClassificationRef'">
            <xsl:call-template name="derivativeClassificationRef"></xsl:call-template>
          </xsl:when>		
          <xsl:when test="$templateName='derivativeClassificationRefId'">
            <xsl:call-template name="derivativeClassificationRefId"></xsl:call-template>
          </xsl:when>				
  ...
  <xsl:template match="*|@*">
    <Root>
      <xsl:choose>
        ...
        <xsl:when test="$namedTemplate='brLevelledParaNumber'">
          <xsl:call-template name="brLevelledParaNumber"></xsl:call-template>
        </xsl:when>			  
        ...
        <xsl:when test="$namedTemplate='derivativeClassificationRef'">
          <xsl:call-template name="derivativeClassificationRef"></xsl:call-template>
        </xsl:when>		
        <xsl:when test="$namedTemplate='derivativeClassificationRefId'">
          <xsl:call-template name="derivativeClassificationRefId"></xsl:call-template>
        </xsl:when>			  
  ...
  <xsl:template name="derivativeClassificationRef">
	  <xsl:variable name="ref" select="."></xsl:variable>
	  <xsl:text> [</xsl:text>
	  <xsl:for-each select="//identAndStatusSection/*/derivativeClassification/classificationActionGroup[@id=$ref]/classificationAction">
		  <xsl:value-of select="concat(@actionIdentType,' ',@year,'-',@month,'-',@day,' ',businessUnit/businessUnitName,' / ',derivativeSource)"/>
      </xsl:for-each>
      <xsl:text>] </xsl:text>
  </xsl:template>		
...
<xsl:template name="brLevelledParaNumber">
    <xsl:param name="node" select="." />
    <xsl:param name="number">
          <xsl:value-of select="count($node/preceding-sibling::brLevelledPara)+1" />
    </xsl:param>
    <xsl:variable name="c">
          <xsl:value-of select="count($node/parent::*/preceding-sibling::brLevelledPara)+1" />
    </xsl:variable>
    <xsl:choose>
      <xsl:when test="$node/parent::brLevelledPara">
        <xsl:call-template name="brLevelledParaNumber">
          <xsl:with-param name="node" select="$node/parent::brLevelledPara" />
